...
//
// On this branch, the engine uses maxIterations=5 per step, so after tool
// execution the executor loops back for another LLM call. A text-only
// response ends the step.
// ============================================================================
...
describe('WorkflowEngine Integration – File Operations', () => {
  ...
  it('should complete a read → write multi-step workflow', async () => {
    ...
    provider.setResponses([
      ...
      // 2) Step 1, iter 1: model calls read_file
      {
        content: '',
        finishReason: 'stop',
        toolCalls: [{ id: 'tc1', function: { name: 'read_file', arguments: { path: 'input.txt' } } }],
      },
      // 3) Step 1, iter 2: model sees tool result, responds with text (ends step)
      {
        content: 'Read input.txt successfully: hello world',
        finishReason: 'stop',
      },
      // 4) Step 2, iter 1: model calls write_file
      {
        content: '',
        finishReason: 'stop',
        toolCalls: [{ id: 'tc2', function: { name: 'write_file', arguments: { path: 'output.txt', content: 'HELLO WORLD' } } }],
      },
      // 5) Step 2, iter 2: text answer (ends step)
      {
        content: 'Written HELLO WORLD to output.txt',
        finishReason: 'stop',
      },
  ...
  it('should handle read_file failure gracefully', async () => {
    ...
    provider.setResponses([
      ...
      // Step, iter 1: tool call for missing file
      {
        content: '',
        finishReason: 'stop',
        toolCalls: [{ id: 'tc1', function: { name: 'read_file', arguments: { path: 'missing.txt' } } }],
      },
      // Step, iter 2: model sees error, responds with text
      {
  ...
  it('should execute read in single step workflow', async () => {
    ...
    provider.setResponses([
      // Planning
      {
        content: JSON.stringify({
          goal: 'Process CSV',
          steps: [
            { id: 'step-1', description: 'Read CSV', expectedTools: ['read_file'], dependsOn: [], successCriteria: 'Done' },
          ],
          maxSteps: 1,
        }),
        finishReason: 'stop',
      },
      // Step, iter 1: read_file
      {
        content: '',
        finishReason: 'stop',
        toolCalls: [
      ...
      // Step, iter 2: text answer
      {
        content: 'CSV data: 2 records found.',
        finishReason: 'stop',
      },
      // Workflow final answer
      {
