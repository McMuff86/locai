...
import type { AgentNodeData, NodeRunStatus } from '@/lib/flow/types';
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 px-3 py-2.5">
        <div className="rounded-md border border-fuchsia-500/20 bg-fuchsia-500/5 px-2 py-1.5 text-[11px] leading-relaxed text-foreground/80">
      ...
      <div className="flex items-center justify-between border-t border-border/60 px-3 py-1.5 text-[10px] text-muted-foreground">
        <span>{data.config.model || 'llama3'}</span>
        <span className="inline-flex items-center gap-1">
          <Wrench className="h-3 w-3" />
          {data.config.tools.length} tools
        </span>
      </div>

      <Handle type="target" position={Position.Left} className="!h-3 !w-3 !bg-fuchsia-400" />
      <Handle type="source" position={Position.Right} className="!h-3 !w-3 !bg-fuchsia-400" />
    </div>
  );
}
